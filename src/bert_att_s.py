...
class Bert_att(nn.Module):
    ...
    def forward(self, input_1, input_2):
        ...
        output_1, output_2, Att = self.graph_match(output_1, output_2) 
        ...
        if self.training:
            ...
            output_2_doc = output_2[:,-1,:]
            output_2_doc = output_2_doc.unsqueeze(1)
            output_2_doc = output_2_doc.expand(output_1.size())
            Att = self.cos(output_1, output_2_doc)
            output = torch.cat((output_1, output_2_doc), dim=2)
            output = self.r(self.fd(output))
            output = torch.squeeze(self.ff(output))
